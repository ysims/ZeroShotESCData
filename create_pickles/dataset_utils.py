...
def waveform_to_examples(data, sample_rate, num_mel_bins, length=5):
    ...
    if len(data) < length * SAMPLE_RATE:
        data = np.pad(data, (0, length * SAMPLE_RATE - len(data)), "constant")
    else:
        data = data[:length * SAMPLE_RATE]
...
def wavfile_to_examples(wav_file, num_mel_bins, length=5):
    ...
    return waveform_to_examples(samples, sr, num_mel_bins, length)
